...
from feature_lens.utils.device import set_device
...
def test_sparsity():
    ...
    for layer in range(12):
        ...
        assert sparsity < 0.3, f"Layer {layer} Att-SAE sparsity: {sparsity}"

        # Transcoder
        hook_name = f"blocks.{layer}.ln2.hook_normalized.{SAE_NODE_ACT_NAME}"
        act = cache[hook_name]
        sparsity = (act != 0).float().mean()
        assert sparsity < 0.3, f"Layer {layer} MLP-TC sparsity: {sparsity}"
