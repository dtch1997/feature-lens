...
def query_user_for_sae_metadata():
    ...
    upstream_release = st.selectbox('Select upstream SAE release', ALL_RELEASES, index = ALL_RELEASES.index("gpt2-small-res-jb"))
    upstream_layer = st.selectbox('Select upstream SAE layer', ALL_LAYERS, index = ALL_LAYERS.index(8))
    upstream_metadata = get_sae_metadata(upstream_release, upstream_layer)

    st.subheader('Downstream SAE')
    downstream_release = st.selectbox('Select downstream SAE release', ALL_RELEASES, index = ALL_RELEASES.index("gpt2-small-hook-z-kk"))
    downstream_layer = st.selectbox('Select downstream SAE layer', ALL_LAYERS, index = ALL_LAYERS.index(8))
...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        st.write("This dashboard allows you to explore the attribution of a downstream feature in a model, "
                    "using a pair of SAEs (Sparse Attributional Explainer).")
        st.write("To get started, please select the SAEs you would like to use for attribution.")
        ...
        n_features = 49512
        downstream_feature = st.number_input('Select downstream feature', min_value=0, max_value=n_features-1, value=16513)
        downstream_feature = int(downstream_feature)

        custom_prompt = st.text_area('Enter a custom prompt (leave blank to use max-activating example)', value="")
    ...
    feature_iframe = get_feature_iframe_src_html(downstream_metadata, downstream_feature)
    components.iframe(feature_iframe, height = 500, width = 1080)

    feature_info = get_feature_info("gpt2-small", downstream_metadata.neuronpedia_layer, downstream_feature)
    ...
    with model.saes([upstream_sae, downstream_sae]):
        upstream_cache = get_sae_cache_for_target_feature_as_metric(model, handler, downstream_metadata.hook_point, downstream_feature)
    # Compute attribution
    attrib = compute_attribution(upstream_metadata.hook_point, clean_cache = upstream_cache, corrupt_cache = None)
    ...
    fig = px.bar(topk_df, x = "feature", y = "attribution", title="Top 10 attribution scores", category_orders={"feature": topk_df["feature"].tolist()})
    fig.update_layout(
        xaxis_title="Feature",
    ...
    for i, (upstream_feature, score) in enumerate(zip(topk_features, topk_scores)):
        feature_iframe = get_feature_iframe_src_html(upstream_metadata, upstream_feature)
