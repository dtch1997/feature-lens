...
class TextHandler: 
    ...
    def get_logits(
        ...
        if input != "clean":
            raise ValueError("TextHandler only supports clean input as it is unpaired. ")
        return model(self.curr_tokens, **kwargs)
    
    def get_metric(self, logits: Logits) -> Metric:
        # Return cross-entropy loss from logits
        logprobs = logits.log_softmax(dim=-1)
        selected_logprobs = eindex.eindex(logprobs, self.next_tokens, "batch seq [batch seq]")
...
def as_dataframe(handler: TextHandler) -> pd.DataFrame:
